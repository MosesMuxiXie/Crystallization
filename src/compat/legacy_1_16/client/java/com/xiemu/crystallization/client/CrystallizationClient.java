...
public final class CrystallizationClient implements ClientModInitializer {
	public static final KeyMapping TOGGLE_KEY = KeyBindingHelper.registerKeyBinding(
			new KeyMapping(
					"key.crystallization.toggle",
					InputConstants.Type.KEYSYM,
					GLFW.GLFW_KEY_F,
					"key.category.crystallization.general"
			)
	);
	...
	public static void setEnabled(boolean enabled, boolean showMessage) {
		...
		if (showMessage) {
			...
			if (client.player != null) {
				String key = enabled
						? "message.crystallization.enabled"
						: "message.crystallization.disabled";
				client.player.displayClientMessage(
						new TranslatableComponent(key).withStyle(ChatFormatting.GREEN),
						true
				);
			}
		}
	...
	public static Component shortcutText() {
		return new TranslatableComponent(
				"option.crystallization.shortcut",
				TOGGLE_KEY.getTranslatedKeyMessage()
		);
	}
