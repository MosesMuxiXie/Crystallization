...
public final class CrystallizationConfigScreen extends Screen {
	...
	protected void init() {
		...
		this.addRenderableWidget(new Button(left, firstRow, BUTTON_WIDTH, 20, toggleText(), button -> {
			CrystallizationClient.setEnabled(!CrystallizationConfig.isEnabled(), false);
			button.setMessage(toggleText());
		}));
		this.addRenderableWidget(new Button(left, firstRow + 24, BUTTON_WIDTH, 20,
				CrystallizationClient.shortcutText(),
				button -> this.minecraft.setScreen(new KeyBindsScreen(this, this.minecraft.options))));
		this.addRenderableWidget(new Button(left, firstRow + 60, BUTTON_WIDTH, 20,
	...
	private Component toggleText() {
		return CommonComponents.optionStatus(
				...
				CrystallizationConfig.isEnabled()
		);
	}
